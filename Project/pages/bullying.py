...
def load_lottiefile(filepath:str):
    with open(filepath,"r") as f:
        return json.load(f)
lottie_variable5=load_lottiefile("5.json")   
lottie_variable6=load_lottiefile("6.json")  
lottie_variable7=load_lottiefile("7.json")  
lottie_variable8=load_lottiefile("8.json")   

#header
st.subheader("We are here to help you")
st.title("A step forward to eradicate bullying")
...
with st.container():
    st.write("---")
    st_lottie(lottie_variable5,height=400,key="catu5") 
    st.write("##")
    st.subheader("we designed EXPRESSION EXCAVATOR to detect and combat bullying in the digital age. It stands as a powerful ally for individuals seeking to create safer online environments. We have utilized the power of technology to protect users from the harmful effects of bullying. We are committed to fostering a more respectful and inclusive online community for all.")
with st.container():
    st.write("---")
    st.subheader("sample text")
    file=open("bully.txt", 'r')
    txt=file.read()
    st.write(txt)
...
with st.container():
    ...
    with left_column:
        st_lottie(lottie_variable6,height=400,key="catu6")              
...
with st.container():
    ...
    with left_column:
        ...
        with right_column:
         st_lottie(lottie_variable7,height=500,key="catu7")    
